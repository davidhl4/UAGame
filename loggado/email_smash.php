<?php
	...
	$q = mysqli_query($link,"SELECT * FROM smash WHERE id = '6' ");
	$w = mysqli_query($link,"SELECT * FROM usuarios WHERE usuario = '$username' OR email = '$email' ");
	$x = mysqli_query($link,"SELECT * FROM smash WHERE usuario = '$username' OR email = '$email' ");
	...
	if( mysqli_num_rows($w) == 1){
		...
		if( mysqli_num_rows($x) == 0){
			...
			if( mysqli_num_rows($q) == 0){
				...
				mysqli_query($link,"INSERT INTO smash(usuario,email) VALUES ('$username','$email')");
